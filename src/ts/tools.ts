...
export class Color {
	...
	static hexToRGBA(hexColor: string): RGBAColor {
		...
		_hexColor = _hexColor.length === 3
			? `${hexColor[ 0 ]}${hexColor[ 0 ]}${hexColor[ 1 ]}${hexColor[ 1 ]}${hexColor[ 2 ]}${hexColor[ 2 ]}${hexColor[ 3 ] || "f"}${hexColor[ 3 ] || "f"}`
			: _hexColor
	...
	static colorType(rgbaColor: RGBAColor | string): "rgba" | "hex" {
		return (rgbaColor as RGBAColor).red && (rgbaColor as RGBAColor).green && (rgbaColor as RGBAColor).blue
			? "rgba"
			: "hex"
	}
	static textColor(color: RGBAColor | string) {
		const { red, green, blue } = this.colorType(color) === "hex"
