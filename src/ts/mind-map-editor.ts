...
async function exportAs(format: MindMapExportingFormat) {
	...
	await page.goto(location.pathname.slice(navigator.platform === "Win32" ? 1 : 0, location.pathname.lastIndexOf("/")) + "/mind-map-exporter.html")
	...
	const out = format === "pdf"
		...
		: (await page.screenshot({ clip, encoding: "base64", type: format, ...(format === "jpeg" ? { quality: 95 } : {}) })) as string | Buffer
	await browser.close()
...
function joinAll() {
	...
	if (lines.length === data.links.length) return;
		...
		for (i = i; i < lines.length; i++) lines[ i ].remove()
}
function rejoinAll() {
...
function addListeners(bubble: Bubble) {
	...
	bubble.addEventListener("update-data", () => {
		if (data.bubbles.some(b => bubble.id === b.id))
			...
			data.bubbles.push(bubble.toJSON())
	})
}
...
const updateBubble = (bubble: Bubble) =>
	bubble.update(data.settings as any)

const updateAllBubbles = () =>
	([ ...document.querySelectorAll("map-bubble") ] as Bubble[])
		.forEach(updateBubble)
...
window.addEventListener("dblclick", (e: MouseEvent) => !e.composedPath().some(f => (f as Element).nodeName === "MAP-BUBBLE") ? newBubble(e) : null)