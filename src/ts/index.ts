...
async function openFile(path: string): Promise<void> {
	if (!existsSync(path)) return void Swal.fire({
...
window.addEventListener("load", async () => {
	if (!existsSync("./recents.json") || (await readFile("./recents.json", "utf8")) === "")
		...
		fileListE.innerHTML += `
			<div class="file" title="${filename}">
				...
				<div class="info">
					...
					<span class="description">${moment(timestamp).locale(navigator.language).fromNow()} | ${fileTypes[ p.extname(filename) ] || "[{(unknown)}]"}</span>
				</div>
			</div>`
	})
